...
struct TweetService {
    ...
    func uploadTweet(caption: String, type: UploadTweetConfiguration, completion: @escaping(Error?, DatabaseReference) -> Void) {
        ...
        switch type {
        case .tweet:
            REF_TWEETS.childByAutoId().updateChildValues(values) { (error, dbRef) in
                guard let tweetID = dbRef.key else { return }
    ...
    func fetchTweets(forUser user: User, completion: @escaping ([Tweet]) -> Void) {
        ...
        REF_USER_TWEETS.child(user.uid).observe(.childAdded) { snapshot in
            let tweetID = snapshot.key
            self.fetchTweet(withID: tweetID) { tweet in
                tweets.append(tweet)
                completion(tweets)
            }
        }
    ...
    func fetchTweet(withID tweetID: String, completion: @escaping (Tweet) -> Void) {
    ...
    func likeTweet(tweet: Tweet, completion: @escaping(DatabaseCompletion)) {
        ...
        if tweet.didLike {
            ...
            REF_USER_LIKES.child(uid).updateChildValues([tweet.tweetID: 1]) { (err, dbref) in
